...
import { getWishCart } from '../../utils/wishCart';
import { MdOutlineAddLocationAlt, MdOutlineContactPage } from 'react-icons/md';
import { FaUsers } from 'react-icons/fa';

const WishCart = () => {
    const navigate = useNavigate()
    const [addToWishCart, setAddToWishCart] = useState([]);

    useEffect(()=> {
        const savedWishCart = getWishCart();
        setAddToWishCart(savedWishCart);
    }, [])
    return (
    <div className="grid grid-cols-1 gap-4">
      {addToWishCart.map((book) => (
        <div
          key={book.bookId}
